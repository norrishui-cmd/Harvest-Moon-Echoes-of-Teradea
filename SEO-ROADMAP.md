...
- 补充角色入口、五类关键道具、三类特殊玩法、Demo 状态、实时游戏状态和首支预告片资料。
- 原 Items 模板升级为可索引数据 Hub，新增集中式 `current-game-data.json` 作为后续动态更新源。
- 所有状态页包含最后核验日期、官方来源和仍待确认边界。

## V7 非薄内容扩展（2026-07-25）

- 基线：254 个 HTML 页面、242 条可索引 URL。
- 新增 4 个结构不同的内容中心：发售前事实、购买决策、探索规划、世界与剧情档案。
- 新增 24 个英语深度页面，覆盖时间线、确认/未确认边界、来源核验、实体版和价格、零售商与 Lupo 特典、运输和区域兼容、营地与远征、动物能力、岛屿、矿洞、野外风险及四个村庄/森林/Guardian Spirits 档案。
- 新增德语 8 页、日语 8 页，并建立 8 组 en/de/ja/x-default 互惠 hreflang。
- 当前达到 298 个 HTML 页面、286 条可索引 URL；12 个缺少真实数据的旧模板继续 noindex。
- 新增正文质量门：英语叶子页至少 190 个可见词、至少四个内容段、至少一个一方来源，并检查页面间过度相似。
- 距离 1000 条可索引 URL 尚差 714 条。下一阶段仍依赖新的官方角色/系统资料或合法预览实测数据；不把“尚未公布”拆成多条占位页。

## V8 角色、地点与界面证据扩展（2026-07-25）

- 基于最新零售商商品图库补充 7 个已出现名字的角色：Lorelei、Bryce、Mara、Cindy、Amad、Lily、Rick。
- 补充 4 个可见地点：Wolf Hill、Bloomfield Park、Maple Mart、Tornado Island。
- 新增 `/interface/` 内容中心，以及 DocPad、Photo Mode、换装、选歌、任务追踪、时钟/日历/天气、地图/区域标签 7 个系统页。
- 对零售商截图证据与 Natsume 一方资料分层标注；截图只证明可见名字、文字和控件，不推断人物身份、恋爱状态、任务步骤或最终平台操作。
- 德语与日语各新增 6 个高价值本地化页面，形成 6 组 en/de/ja/x-default 互惠 hreflang。
- 当前达到 329 个 HTML 页面、317 条可索引 URL；12 个缺少真实数据的旧模板继续 noindex。
- 全站语言下拉与 AdSense 覆盖 329/329；新增页面均有 hub 回链、唯一元数据、明确未知边界和发布后数据升级路径。
- 距离 1000 条可索引 URL 尚差 683 条。下一批优先等待正式角色介绍、平台商店技术字段、Demo/预览或合法实测数据。
